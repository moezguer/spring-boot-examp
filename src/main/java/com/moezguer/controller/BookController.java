...
public class BookController {
    ...
    // Find
    @GetMapping("/books")
    List<Book> findAll(@RequestParam(value = "name",
    ...
    List<Book> findAllWithPriceFilter(@RequestParam(value = "price",
    ...
    //return 201 instead of 200
    @ResponseStatus(HttpStatus.CREATED)
    List<Book> newBook(@RequestBody final List<Book> newBook) {
        return newBook.stream().map(book -> repository.save(book)).collect(Collectors.toList());
    }
    ...
    Book patch(@RequestBody final Book update, @PathVariable final Long id) {
